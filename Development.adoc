...
+
----------------------------------
oc new-project user-group-operator
----------------------------------
...
+
------------------------------------------------------------
oc start-build user-group-operator --from-dir=. --follow
------------------------------------------------------------
...
--set=image.repository=$(oc get imagestream user-group-operator -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}') \
| oc apply -f -
--------------------------------------------------------------------------------
